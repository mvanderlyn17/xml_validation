    ...
    for record in event['Records']:
        ...
        if not(output ==None):
            ...
            if(validity_status =="Valid"):
                ...
                try:
                    s3_client.delete_object(Bucket='gen3-interns-'+content_provider+'failures', Key=''+package_name+'_logs.txt')
                    s3_client.upload_file('/tmp/'+package_name+'_logs.txt', 'gen3-interns-'+content_provider+'total', ''+package_name+'_logs.txt')
                except:
                    s3_client.upload_file('/tmp/'+package_name+'_logs.txt', 'gen3-interns-'+content_provider+'total', ''+package_name+'_logs.txt')
            else:
                print('Status: Invalid')
                s3_client.upload_file('/tmp/'+package_name+'_logs.txt', 'gen3-interns-'+content_provider+'failures', ''+package_name+'_logs.txt')
